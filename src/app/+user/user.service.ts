...
export class UserService implements IUserService {
    ...
    private _recentUsers: BehaviorSubject<IUser[]> = new BehaviorSubject([]);
    users: Observable<IUser[]> = this._recentUsers.asObservable();
    errorMessage: string;
    ...
    constructor(private http: Http) {
        ...
        let ra: IUser[] = this.allUsers.slice(0, 4);
    ...
    get(name: string): Promise<IUser> {
        ...
        while (idx < this.allUsers.length) {
            let user: IUser = this.allUsers[ idx ];
            if (user.name === name) {
                rval = user;
                break;
            }
