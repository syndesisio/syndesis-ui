...
export class ConnectionService implements IConnectionService {
    ...
    private _connections: BehaviorSubject<IConnection[]> = new BehaviorSubject([]);
    private _recentConnections: BehaviorSubject<IConnection[]> = new BehaviorSubject([]);
    connections: Observable<IConnection[]> = this._recentConnections.asObservable();
    errorMessage: string;
    recentConnections: Observable<IConnection[]> = this._recentConnections.asObservable();
    ...
    constructor(private http: Http, private forge: Forge) {
        ...
        let ra: IConnection[] = this.allConnections.slice(0, 4);
    ...
    get(name: string): Promise<IConnection> {
        let rval: IConnection = null;
        let idx: number = 0;
        while (idx < this.allConnections.length) {
            let connection: IConnection = this.allConnections[ idx ];
     ...
     * @return Promise<Connection> - Returns a Promise. Should perhaps return an Observable instead.
     */
    resolveConnectionInfo(connection: IConnection): Promise<IConnection> {
     ...
     * @return Promise<Connection> - Returns a Promise. Should perhaps return an Observable instead.
     */
    search(query: string): Promise<IConnection> {
        let rval: IConnection = null;
        // Do stuff here
     ...
     */
    private loadConnectionsFromLocalStorage(): IConnection[] {
     ...
     */
    private storeConnectionsInLocalStorage(connections: IConnection[]): void {
